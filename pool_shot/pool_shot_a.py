...
class PoolSimulation:
    ...
    def create_table(self):        
        ...
        for pos in pocket_positions:
            ...
            pocket_shape.color = (0, 0, 0, 255)  # Black color for pockets
            self.space.add(pocket_body, pocket_shape)
            self.pockets.append(pocket_shape)
        
        # Set up collision handler for pockets
        handler = self.space.add_collision_handler(0, 0)  # Default collision type for balls and pockets
        handler.begin = self.pocket_collision

    def pocket_collision(self, arbiter, space, data):
        """Handle collision between a ball and a pocket."""
        ball_shape = arbiter.shapes[0] if arbiter.shapes[0] not in self.pockets else arbiter.shapes[1]
        if ball_shape.body in [self.cue_ball_body, self.top_ball_body, self.bottom_ball_body] or ball_shape in self.space.shapes:
            self.pocketed_balls += 1
            self.space.remove(ball_shape.body, ball_shape)  # Remove the ball from simulation
